...
if __name__ == '__main__':
    ...
    parser.add_argument('-save_path', dest='save_path', default="global_mosaic/stitched_"+datetime.now().strftime('%Y-%m-%d_%H-%M-%S'), type=str, help="path to save result")
    ...
    for image_path in image_paths:
        if not os.path.exists(image_path):
            print('{0} directory doesnt not exists!'.format(image_path))
            continue
        ...
        print("reading image from {0}".format(image_path))
        ...
        h_time = time.time()
        H = surf(result_gry, image_gray)
        elapsed_time_h = time.time()-h_time

        H_flat = np.array(H).flatten().astype(np.float64)
        print(H_flat)
        print(args.save_path)        
        with open(save_path+"/H_surf_global_from_mini.csv", 'a') as f1:
           wr = csv.writer(f1, delimiter=",", escapechar = ",", quoting = csv.QUOTE_NONE)
           wr.writerow(H_flat)

        with open(save_path+"/H_surf_global-from_mini_time_elapsed.csv", 'a') as f2:
